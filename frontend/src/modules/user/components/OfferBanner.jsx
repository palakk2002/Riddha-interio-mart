  ...
  return (
    <section className="max-w-7xl mx-auto px-4 md:px-12 py-4">
      <div className="flex flex-row h-32 sm:h-40 md:h-64 rounded-xl md:rounded-[2.5rem] overflow-hidden shadow-lg border border-soft-oatmeal/10 bg-white">
        {/* Left Content (30-35%) with White Background */}
        <div className="w-[38%] md:w-[30%] bg-white flex flex-col justify-center px-4 md:px-12 py-2 md:py-8 space-y-1.5 md:space-y-4">
          <motion.h3 
            initial={{ opacity: 0, x: -20 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="text-[14px] sm:text-lg md:text-3xl font-black text-deep-espresso leading-[1.2] md:leading-tight uppercase tracking-tight"
          >
          ...
          >
            <Link to="/products" className="text-[10px] sm:text-xs md:text-lg font-bold text-warm-sand hover:text-deep-espresso uppercase tracking-[0.1em] md:tracking-widest flex items-center gap-2 transition-colors">
