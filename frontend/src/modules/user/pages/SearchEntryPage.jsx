...
const SearchEntryPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Search Header */}
      <div className="sticky top-0 z-50 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-4">
        <button 
          onClick={() => navigate(-1)}
            ...
            <div className="flex flex-wrap gap-2">
              {recentSearches.map((s, i) => (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 rounded-full text-xs font-bold text-gray-600 transition-colors border border-gray-100"
                >
          ...
          <div className="flex flex-wrap gap-3">
            {popularSearches.map((s, i) => (
              <button
                key={i}
                 onClick={() => handleSearch(s)}
                 className="flex items-center gap-3 px-4 py-3 bg-white border border-gray-100 hover:border-[#189D91]/30 hover:bg-gray-50 rounded-xl text-[13px] font-bold text-gray-700 transition-all shadow-sm group"
               >
