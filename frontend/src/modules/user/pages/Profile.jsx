...
const Profile = () => {
  ...
  const menuItems = [
    { icon: <FiUser />, title: "My Profile", subtitle: "View and edit your personal details", link: "#" },
    ...
    { icon: <FiHeart />, title: "Wishlist", subtitle: "Items you've saved for later", link: "/products" },
    { icon: <FiSettings />, title: "Account Settings", subtitle: "Update your profile and security", link: "#" },
  ];
  ...
  return (
    ...
    >
      {/* Header / Profile Info */}
      <div className="bg-white border-b border-soft-oatmeal/10 pt-8 pb-8 px-6 md:px-12 md:pt-16 md:pb-12">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-4 md:gap-12">
          <div className="relative group">
            <div className="h-28 w-28 md:h-36 md:w-36 rounded-full bg-soft-oatmeal/10 flex items-center justify-center border-4 border-warm-sand/5 overflow-hidden shadow-2xl">
              <FiUser className="h-14 w-14 md:h-20 md:w-20 text-deep-espresso/20" />
            </div>
          </div>

          <div className="text-center md:text-left space-y-1 md:space-y-3">
            <h1 className="text-4xl md:text-5xl font-black text-deep-espresso tracking-tighter uppercase">{user.name}</h1>
            <p className="text-xs md:text-sm text-gray-400 font-bold uppercase tracking-[0.2em]">{user.email}</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 md:py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border-t border-l border-soft-oatmeal/10">
          {menuItems.map((item, index) => (
            <Link 
              key={index} 
              to={item.link}
              className="group p-5 md:p-6 border-r border-b border-soft-oatmeal/10 hover:bg-soft-oatmeal/5 transition-all duration-500"
            >
              <div className="flex flex-row items-center gap-5 md:flex-col md:items-start md:gap-4">
                <div className="h-10 w-10 flex items-center justify-center text-warm-sand group-hover:scale-110 transition-transform duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-t border-l border-soft-oatmeal/10">
            {[
              ...
              <Link 
                key={i} 
                to={item.link}
                className="flex items-center justify-between p-5 border-r border-b border-soft-oatmeal/10 hover:bg-soft-oatmeal/5 transition-colors group"
              >
        ...
        <div className="mt-20">
          <button 
            ...
            className="w-full md:w-auto px-16 py-6 bg-deep-espresso text-white font-black uppercase tracking-[0.3em] text-xs hover:bg-warm-sand transition-all duration-500 shadow-2xl flex items-center justify-center gap-4 group"
          >
            LOGOUT 
