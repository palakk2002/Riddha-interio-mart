...
const AddFavouriteCategoryItemPage = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (parsed.tabs.length > 0) {
        setItem(prev => ({ ...prev, category: parsed.tabs[0] }));
      }
    } else {
      setData(manageFavouriteCategoriesData);
      if (manageFavouriteCategoriesData.tabs.length > 0) {
        setItem(prev => ({ ...prev, category: manageFavouriteCategoriesData.tabs[0] }));
      }
    }
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    const updatedData = {
      ...data,
      categories: [...data.categories, newItem],
    };
